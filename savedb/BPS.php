<?php
            ...
            $sqlCheck = "SELECT * FROM bank_payment_sheet WHERE Datee = '$Month - $Year'";
            $resultCheck = $conn->query($sqlCheck);
            
            if ($resultCheck->num_rows > 0) {
                // Date already exists, do something (e.g., display a message)
                echo '<div class="alert alert-warning mt-2 mx-auto" role="alert"> Payroll Sheet For this Month already is exists in the database. </div>';
            } 
            else {
            ...
            if ($result->num_rows > 0) {
                while ($row = $result->fetch_assoc()) {
                    ...
                    $query = "INSERT INTO `bank_payment_sheet`(`Employee_ID`, `Name`, `Bank_Name`, `Account_Number`, `Net_Pay`, `Datee`)
                    VALUES ('$empid','$names','$Bnames','$BAcc','$net','$Month - $Year')";
